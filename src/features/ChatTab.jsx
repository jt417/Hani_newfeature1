...
export default function ChatTab({
  ...
  return (
    <div className="flex flex-col h-full bg-surface-light relative pb-[80px] pt-2">
          ...
          <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
            <div className={`flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
              <div className={`p-3.5 max-w-[260px] text-[13px] leading-relaxed shadow-sm ${msg.sender === 'user'
                ? 'bg-surface-muted text-primary rounded-2xl rounded-tr-sm font-medium'
      ...
      <div className="bg-white p-3 flex gap-2 items-center z-10 shrink-0 shadow-[0_-2px_10px_rgba(0,0,0,0.02)]">
        <input
          ...
          className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-primary transition"
          value={chatInput}
        ...
        <button
          ...
          className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${chatInput.trim() && !isTyping ? 'bg-primary text-white shadow-md' : 'bg-gray-100 text-gray-400'}`}
        >
